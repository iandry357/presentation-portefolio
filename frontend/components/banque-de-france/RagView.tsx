...
import { fetchBdfRag, RagResponse } from '@/lib/banqueApi';
import { Badge } from '@/components/ui/badge';

const SUGGESTED_QUESTIONS = [
  "Quelles sont les dernières actualités de la Banque de France ?",
  "Quelles sanctions récentes l'ACPR a-t-elle prononcées ?",
  "Quelles évolutions réglementaires bancaires sont en cours ?",
  "Quels sont les enjeux actuels de supervision bancaire en France ?",
];
...
export default function RagView() {
  ...
  const handleSubmit = async (q?: string) => {
    ...
    setResult(null);
    try {
      const res = await fetchBdfRag(query, 5);
      setResult(res);
    } catch (e: unknown) {
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-1">Ask AI — Banque de France</h2>
        <p className="text-sm text-gray-500">
          Posez une question sur l'actualité de la Banque de France et de l'ACPR. Le modèle interroge
          la base de veille via RAG (décisions ACPR non incluses — voir l'onglet Classification).
        </p>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          placeholder="Ex: Quelles sont les dernières actualités de la Banque de France ?"
