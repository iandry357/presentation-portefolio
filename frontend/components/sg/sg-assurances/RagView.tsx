...
import { fetchSgRag, RagResponse } from '@/lib/sgApi';
import { Badge } from '@/components/ui/badge';

const SUGGESTED_QUESTIONS = [
  "Quelles sont les dernières actualités sur l'assurance auto SG ?",
  "Quelles évolutions réglementaires impactent SG Assurances ?",
  "Quels sont les nouveaux produits d'assurance habitation SG ?",
  "Comment SG Assurances gère-t-elle les sinistres climatiques ?",
];
...
export default function RagView() {
  ...
  const handleSubmit = async (q?: string) => {
    ...
    setResult(null);
    try {
      const res = await fetchSgRag(query, 5);
      setResult(res);
    } catch (e: unknown) {
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-1">Ask AI — SG Assurances</h2>
        <p className="text-sm text-gray-500">
          Posez une question sur les actualités SG Assurances. Le modèle interroge la base de veille via RAG.
        </p>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          placeholder="Ex: Quelles sont les dernières actualités SG Assurances ?"
