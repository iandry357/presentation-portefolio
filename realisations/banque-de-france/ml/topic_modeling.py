"""
Topic Modeling — Banque de France
LDA sur articles de veille BigQuery (source=google_news uniquement,
decisions ACPR exclues — cf. bq_client.py) + labeling LiteLLM (Mistral/Gemini fallback)
"""
...
# ─────────────────────────────────────────
# Stopwords — regulation bancaire / bruit RSS
# ─────────────────────────────────────────
NEWS_STOP_WORDS = [
    # Entites Banque de France / ACPR — presentes dans quasi tous les articles
    # de ce corpus par construction (flux RSS dedies), donc sans pouvoir
    # discriminant pour separer les topics entre eux.
    "banque", "france", "acpr", "commission", "sanctions",
    # Bruit générique
    ...
    "groupe", "résultats", "trimestre", "rapport",
...
def _generate_label(keywords: list) -> str:
    prompt = (
        f"Tu analyses des sujets liés à la régulation bancaire française, "
        f"en lien avec la Banque de France et l'ACPR. "
