"""
Collecteur Google News RSS — Banque de France / ACPR.
Collecte les actualités Banque de France et ACPR via feedparser + Trafilatura.
Deux flux séparés : actualités générales Banque de France, et actualités ACPR/sanctions.
"""
import hashlib
import logging
...
def _collect_feed(feed_cfg: Dict) -> List[Dict]:
    ...
    for entry in feed.entries[:max_articles]:
        ...
        title = entry.get("title", "")
        doc_id = f"banque_{name}_{hashlib.md5(title.encode()).hexdigest()[:16]}"
        ...
        doc = {
            ...
            "title": title,
            "content": _fetch_content(source_href) or _build_content(entry),
            "metadata": {
                "url": entry.get("link", "") or source_href,
...
def collect() -> List[Dict]:
    ...
    logger.info(f"✅ Veille RSS total — {len(all_docs)} articles collectés")
    return all_docs