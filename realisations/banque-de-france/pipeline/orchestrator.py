"""
Orchestrateur principal du pipeline Banque de France.
...
from pipeline.collectors import veille_rss, acpr_pdf_collector
...
def run():
    ...
    logger.info("=" * 60)
    logger.info("🚀 Pipeline Banque de France — Démarrage")
    ...
    veille_docs = veille_rss.collect()
    acpr_docs = acpr_pdf_collector.collect()
    raw_docs = veille_docs + acpr_docs

    logger.info(
        f"📊 Total collecté: {len(raw_docs)} documents "
        f"(Veille: {len(veille_docs)}, ACPR: {len(acpr_docs)})"
    )
