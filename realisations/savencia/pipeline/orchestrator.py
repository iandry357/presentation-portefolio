...
from pipeline.collectors import google_news
...
def run():
    ...
    logger.info("=" * 60)
    logger.info("🚀 Pipeline Savencia Veille — Démarrage")
    ...
    raw_docs = gn_docs
    logger.info(f"📊 Total collecté: {len(raw_docs)} documents (News: {len(gn_docs)})")
