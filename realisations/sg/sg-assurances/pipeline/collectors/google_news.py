"""
Collecteur Google News RSS — SG Assurances.
Collecte les actualités SG Assurances IA/Data via feedparser + Trafilatura.
"""
import hashlib
import logging
...
def _collect_feed(feed_cfg: Dict) -> List[Dict]:
    ...
    for entry in feed.entries[:max_articles]:
        ...
        title = entry.get("title", "")
        doc_id = f"sg_{name}_{hashlib.md5(title.encode()).hexdigest()[:16]}"
